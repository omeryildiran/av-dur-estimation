...
import pandas as pd
from my_staircase_v2 import stairCase
...
stair_intensity = stairCase(init_level=init_level, 
                  init_step=initStep, 
                  name="training", 
...
all_staircases=[stair_intensity]
...
def lapse_rate_cond_generate():
    ...
    all_conds=np.tile(all_conds,(50,1))
...
while not endExpNow and stopped_stair_count!=(len(all_staircases)):
    ...
    intensity = stair.next_trial()
    print(f"intensity: {intensity}")
    pre_dur = np.random.uniform(0.4, 0.75)
    post_dur = np.random.uniform(0.4, 0.75)
    ...
    # audio_stim=audio_cue_gen.whole_stimulus(test_dur=test_dur_s, standard_dur=standard_dur, noise_type="white", intensity=intensity, 
    #                                         rise_dur=rise_dur, order=order,
    #                                         pre_dur=pre_dur, post_dur=post_dur, isi_dur=isi_dur,)
        
    audio_stim= np.concatenate([np.random.uniform(0,1,int(sampleRate*pre_dur)),np.random.uniform(0,1,int(sampleRate*1))*intensity,np.random.uniform(0,1,int(sampleRate*post_dur))])
    ...
    while waitingResponse and not endExpNow:
        ...
        response = responseKeys.getKeys(keyList=['left', 'right','space'], waitRelease=False)
        class simKeys:

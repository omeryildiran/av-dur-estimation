...
One of the 12 participants had a auditory duration discrimination threshold that was five times higher in the low SNR condition than in the high-SNR condition, and was excluded from analysis, so all the analysis conducted regarding 11 participants.
...
Each participant completed three sessions with different ranges of audiovisual conflict: (0, $-167$ and $250$~ms), ($-83$ and $167$~ms), and ($-250$ and $83$~ms). The order of sessions was counterbalanced across participants. As in the unimodal condition, easy lapse-rate trials were included in each condition (14 trials per auditory-noise level $\times$ conflict condition). For each condition, participants completed two interleaved staircases (one 1-down–2-up and one 2-down–1-up), each consisting of 70 trials. Across the 7 conflict levels and 2 auditory-noise conditions, this resulted in a total of 2,156 trials for the main task.
...
This model can be viewed as causally inferring that the probability that the two cues originate from the same source is 100\%, so cues are always integrated regardless of cue-conflict magnitude. 
...
In practice, this probability was approximated using Monte Carlo simulation by repeatedly sampling noisy measurements for the test and standard intervals, applying the model-specific decoding rule, and computing the proportion of samples for which the decoded test duration exceeded the decoded standard duration.
...
\caption{
Conflict-dependent biases in auditory duration perception. Point of subjective equality (PSE) shifts as a function of audiovisual conflict for the low auditory-noise condition (\textbf{A}) and high auditory-noise condition (\textbf{B}), averaged over 11 participants. Positive values indicate biases toward longer perceived auditory duration. Red and blue show empirical data. Other lines indicate model predictions for forced fusion, causal inference, and probabilistic cue switching. The model predictions are jittered horizontally for readability. All model predictions were generated using sensory-noise parameters fitted using the bimodal data. Error bars reflect $\pm$~SEM across participant fits.}
...
% Reproducibility: make_switching_prob_figure.py -> switching_probability_by_noise.{pdf,png}; reads model_fits/boxcarFits/<pid>/<pid>_switchingFree_LapseFix_sharedPrior_fit.json (p_v,low = fittedParams[3], p_v,high = fittedParams[5]).
...
\begin{figure}[H]
    ...
    \caption{\textbf{Fitted switching probabilities increase with auditory noise.} Each colored thin line shows one participant's fitted probability of relying on the visual duration ($p_v$) in the low- and high-auditory-noise conditions, estimated from the probabilistic cue-switching model (boxcar fits). The probability of relying on vision increased with auditory noise in $11$ of $11$ participants (group means $0.25 \rightarrow 0.62$; Wilcoxon signed-rank $p = 0.001$). Black diamonds and line show group means $\pm$ SEM.}
    \label{fig:switchingProb}
\end{figure}
%%% --- end added section ---

\subsubsection{High sensory-encoding noise limits model discriminability}
To explain the lack of evidence for causal inference even at large audiovisual cue conflicts, we evaluated the posterior probability of common cause $P(C=1 \mid m_a, m_v)$ across conflict levels using each participant's fit parameters.
...
Decisions are based on the comparison of posterior estimates, such that the observer concludes that the test is longer when $\hat{s}_t > \hat{s}_s$. Under the assumptions above, the posterior difference reduces to
\[
\hat{s}_t - \hat{s}_s = w (m_t - m_s),
\]
so that the decision is equivalent to comparing the measurements directly, i.e., determining whether $m_t - m_s> 0$. From the modeler's perspective, the response probability is obtained by marginalizing over the measurement noise. Because $m_t$ and $m_s$ are independent random Gaussian variables with equal variance $\sigma^2$, their difference follows
\begin{equation}
m_t - m_s
\sim
\mathcal{N}\!\left(
...
One way to interpret cross-modal biases is as a difference in the modality-specific posterior estimates. In a full Bayesian formulation with modality-specific Gaussian priors, the posterior mean for the auditory estimate in log-duration space would be
\begin{equation}
\mu_{\hat{s},a} = \frac{J_a \cdot \log s_a + J_{p,a} \cdot \mu_{p,a}}{J_a + J_{p,a}},
\end{equation}
and similarly for the visual estimate,
\begin{equation}
\mu_{\hat{s},v} = \frac{J_v \cdot \log s_v + J_{p,v} \cdot \mu_{p,v}}{J_v + J_{p,v}},
\end{equation}
where $J_a = 1/\sigma_a^2$, $J_v = 1/\sigma_v^2$, $J_{p,a}=1/\sigma_{p,a}^2$, and $J_{p,v}=1/\sigma_{p,v}^2$. Under this interpretation, differences in modality-specific prior means or prior strengths could produce an auditory--visual bias even when physical durations are matched.

However, these modality-specific prior parameters were not fit as separate free parameters in our analysis. Instead, their net effect was absorbed into a single cross-modal PSE parameter, $\mu_{cm}$, estimated directly from the cross-modal psychometric function.
...
x_{cm} = \log(s_a/s_v),
\end{equation}
where $s_a$ and $s_v$ are the auditory and visual durations. The response probability was
\begin{equation}
P(\text{"auditory longer"})
=
\frac{\lambda_{cm}}{2}
+ (1-\lambda_{cm})\,
\Phi\left(
\frac{x_{cm}-\mu_{cm}}{\sigma_{cm}}
\right),
\end{equation}
where $\lambda_{cm}$ is the lapse rate, $\sigma_{cm}$ is the fitted psychometric slope, and $\mu_{cm}$ is the fitted cross-modal bias. Thus, $\mu_{cm}=0$ corresponds to no auditory--visual bias, whereas nonzero values indicate that one modality had to be physically longer or shorter for the two intervals to be perceived as equal. This fitted bias is the cross-modal bias that we measured and corrected for in the main bimodal experiment.
...
This expression contains no explicit duration prior. A common prior applied to both intervals would not change the reliability-weighted mean comparison that determines the direction of the response, although it would change the posterior variance. In the fitted forced-fusion model, we therefore used the measurement-noise parameters ($\sigma_a$, $\sigma_v$) to determine the reliability weights, and did not include additional prior-mean or prior-variance parameters.
...
The final auditory-duration estimate is a posterior-weighted average of the fused estimate under a common cause and the auditory estimate under separate causes:
...
\end{equation}
where $J_a = 1/\sigma_a^2$ and $J_v = 1/\sigma_v^2$ denote sensory precisions (with $\sigma_a \in \{\sigma_{a,l}, \sigma_{a,h}\}$ depending on condition). In the segregated case ($C=2$), the observer reports the auditory estimate $m_a$, consistent with the task instruction to judge auditory duration. Note that the flat prior has no effect on either estimate; the MAP estimate with a bounded flat prior is identical to the maximum-likelihood estimates we used above.
...
\begin{figure}
    ...
    \caption{PSE shifts as a function of conflict levels   across different auditory-noise conditions for each participant. Here, the $\sigma$ values were estimated using the bimodal data.}  

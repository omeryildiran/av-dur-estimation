...
We tested three causal-inference models, in which the posterior probability of common cause $p(C=1\mid m_a,m_v)$ was computed in the same way. However, the models differed in how this posterior probability was applied to form the final duration estimate used for the decision. Specifically, the models used the posterior probability to (i) average the estimates under the common- and separate-cause hypotheses weighted by their posterior probabilities, (ii) select the estimate associated with the more probable causal structure, or (iii) stochastically sample between these estimates according to the posterior probability. Each model included a prior probability of common cause, $p_c$, which was fitted to the data.
...
The observer responds ``test longer'' whenever the estimated duration of the test interval exceeds that of the standard interval, i.e., when $\hat{s}_t > \hat{s}_s$. Under the shared-prior Gaussian case above, this is equivalent to $m_t > m_s$.

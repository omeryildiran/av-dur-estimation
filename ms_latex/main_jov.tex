...
\begin{figure}[H]
    ...
    \caption{\textbf{Fitted switching probabilities increase with auditory noise.} Each colored thin line shows one participant's fitted probability of relying on the visual duration ($p_v$) in the low- and high-auditory-noise conditions, estimated from the probabilistic cue-switching model (boxcar fits). The probability of relying on vision increased with auditory noise in all participants (group means $0.25 \rightarrow 0.62$; Wilcoxon signed-rank $p = 0.001$). Black diamonds and line show group means $\pm$ SEM.}
...
We first describe the familiar Gaussian-prior case to introduce precision weighting. The implemented models instead use a bounded uniform (boxcar) prior and posterior-mean readouts, as described in the model-specific sections below.

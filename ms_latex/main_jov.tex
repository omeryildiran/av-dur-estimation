...
\begin{equation}
P(\text{``auditory longer''}\mid s_a,s_v)
=
\iint_{\hat{s}_a(m_a)>\hat{s}_v(m_v)}
p(m_a\mid s_a)\,
p(m_v\mid s_v)\,
dm_a\,dm_v.
\end{equation}
...
\begin{equation}
P(\text{respond ``auditory longer''}) = \frac{\lambda_{cm}}{2} + (1-\lambda_{cm}) \cdot \Phi\left( \frac{\mu_{\hat{s},a} - \mu_{\hat{s},v}}{\sqrt{\sigma_{\hat{s},a}^2 + \sigma_{\hat{s},v}^2}} \right).
\end{equation}
...
Note that we omit an explicit prior over duration in this expression. In our 2IFC task, the same duration prior would be applied to both the standard and test intervals, so it would not affect any duration comparisons made by the participant. By contrast, in the Bayesian causal-inference model below, a bounded duration prior is required to evaluate the marginal likelihoods under the common-cause and separate-causes hypotheses.
...
\text{  } t_{min}<y<t_{max}
\end{equation}
To find the likelihood of a common cause, we need a single integrand bounded between $t_{\min}$ and $t_{\max}$:
\begin{equation}
...
\subsubsection{Product of two Gaussians and the final likelihood}
...
\begin{figure}
    ...
    \caption{\textbf{Parameter-recovery analysis for model validation.} Each row corresponds to a generative model and each column to a model parameter. Synthetic datasets ($N = 50$) were generated from fitted group-level parameter distributions and refit with the same model. Scatterplots show true versus recovered parameter values; dashed diagonals indicate perfect recovery and gray bands show a $\pm 5\%$ tolerance region. Insets report Pearson correlation ($r$) and RMSE. Sensory-noise and lapse parameters were generally well recovered, whereas causal-prior and switching parameters were less constrained.}

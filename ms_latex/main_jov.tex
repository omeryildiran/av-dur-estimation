...
\end{equation}
where $t_t$ and $t_s$ denote the test and standard durations, respectively, and $\Phi$ is the standard normal cumulative distribution function. Figure~\ref{fig:unimodal_data_fits} shows the psychometric-function fits (Panel A) for the auditory (high/low noise) and visual conditions and the unimodal duration-discrimination thresholds (Weber fraction; Panel B).
...
All models assume that internal representations of auditory and visual durations are noisy. To capture Weber-like scaling of temporal uncertainty (scalar variability), we assume that durations are encoded in logarithmic space with additive Gaussian noise: 
...
\label{eq:pc}
\end{align}
where $C = 1$ denotes a common cause. The observer then forms a posterior over the task-relevant latent log-duration, $y_a=\log s_a$, by averaging over two posteriors. The observer's posterior distribution over auditory log-duration is given by:
\begin{align}
p(y_a \mid m_a, m_v)
= {}& p(C = 1 \mid m_a, m_v)\,
      p(y_a \mid m_a, m_v,C=1) \notag\\
&+ p(C = 2 \mid m_a, m_v)\,
   p(y_a \mid m_a,C=2),
\label{eq:auditory_bimodal_estimate}
\end{align}
in which $p(C = 2 \mid m_v, m_a)$ indicates the posterior probability of separate causes. $p(y_a \mid m_v, m_a, C=1)$ and $p(y_a \mid m_a,C=2)$ specify the posteriors over auditory log-duration under each causal scenario. The posterior probability of common cause is computed as:
...
Here, the model assumes that participants infer the stimulus duration based on the inferred causal relationship between the cues presented in the trial. The observer first infers the posterior probability that both cues originate from a common cause by computing $p(C = 1 \mid m_a, m_v)$. Under this model, observers compute two estimates of the auditory log-duration, one based on the common-cause scenario that relies on both the auditory and visual measurements, and another for the separate-causes scenario based on the auditory measurement alone. For each scenario, the model uses the Bayes least-squares (BLS) estimate, that is, the posterior mean under squared-error loss in log-duration space. The final auditory log-duration estimate is a weighted average of these two estimates, given by:
\begin{align}
\hat y_{a}
= {}& p(C=1 \mid m_a,m_v)\,\hat y_{a,C=1}
+ \bigl(1 - p(C=1 \mid m_a,m_v)\bigr)\,
   \hat y_{a,C=2},
\label{eq:estimate_model_avg}
\end{align}
where $\hat y_{a,C=1}$ and $\hat y_{a,C=2}$ denote the posterior-mean estimates of auditory log-duration under the common-cause and separate-causes assumptions, respectively. Because the latent log-duration prior has bounded uniform (boxcar) support, these are means of truncated-normal posteriors rather than maximum a posteriori estimates; their full expressions are given in the Appendix. The corresponding estimate in physical time is $\hat s_a=\exp(\hat y_a)$.
The above computation yields an internal estimate of auditory log-duration for a single interval. In the present two-interval forced-choice task, the same inference process is applied independently to the standard and test intervals, producing estimates $\hat y_{\mathrm{std}}$ and $\hat y_{\mathrm{test}}$. Because exponentiation is monotonic, comparing these log-duration estimates is equivalent to comparing the corresponding physical-duration estimates, $\exp(\hat y_{\mathrm{std}})$ and $\exp(\hat y_{\mathrm{test}})$. The observer therefore responds ``test longer'' whenever
\begin{equation}
\hat y_{\mathrm{test}} > \hat y_{\mathrm{std}}.
\end{equation}
...
\mathbb{I}
\!\left[
\hat y_{\mathrm{test}}(m_{\mathrm{test}})
-
\hat y_{\mathrm{std}}(m_{\mathrm{std}})
>0
\right]
...
We also considered a probabilistic cue-switching model, in which observers based their decision on either the auditory or the visual cue on each trial (see Methods). Instead of computing posterior probabilities of causal structure, this heuristic strategy selects a single cue stochastically. In auditory-noise condition $j\in\{l,h\}$, the decision is based solely on the visual estimate with probability $p_{v,j}$ and solely on the auditory estimate with probability $1-p_{v,j}$. Thus, the model has separate visual-selection probabilities, $p_{v,l}$ and $p_{v,h}$, for the low- and high-auditory-noise conditions. Despite its simplicity, the switching strategy can reproduce cue-integration--like patterns when responses are averaged across trials.
...
\caption{
Measured and predicted shifts of point of subjective equality (PSE) as a function of audiovisual conflict for the low auditory-noise condition (\textbf{A}) and high auditory-noise condition (\textbf{B}), averaged over 11 participants. Positive values indicate biases toward longer perceived auditory duration. Red and blue show empirical data. Other lines indicate model predictions for forced fusion, causal inference, and probabilistic cue switching. The model predictions are jittered horizontally for readability. All model predictions were generated using sensory-noise parameters fitted using the bimodal data. Error bars reflect $\pm$~SEM across participants.}
\label{fig:pseConflictModel}
\end{figure}

We also examined whether the models captured the slope of the fitted psychometric functions. The slope of fitted psychometric functions $\sigma$ indexes sensory encoding precision in log-duration units, with larger values corresponding to lower encoding precision (Figure~\ref{fig:sigmaConflictModel}). The probabilistic cue-switching model appeared to track the conflict-dependent changes in slope more closely than the forced-fusion and causal-inference models. To quantify this, we computed the root mean squared error (RMSE) between the measured slopes and those predicted by each model in both noise conditions. RMSE indeed tended to be lower for the cue-switching model (0.048 log units) than for the causal-inference model (0.066) or the forced-fusion model (0.072).
...
\caption{Measured and predicted slope of psychometric functions. The slope $\sigma$ is shown in log-duration unit as a function of audiovisual conflict for the low auditory-noise condition (\textbf{A}) and high auditory-noise condition (\textbf{B}), averaged over 11 participants. Red and blue points show empirical data (group mean $\pm$~SEM across participants). Other lines indicate model predictions for forced fusion, causal inference, and probabilistic cue switching. Error bars indicate SEM across participants. Larger $\sigma$ values indicate lower duration-discrimination precision.}
...
\begin{equation}
\sigma_{\hat{y}}^2 = \frac{1}{J + J_p}.
\label{eq:posterior_var}
\end{equation}
Here, all quantities ($m$, $\hat{y}$, $\mu_p$, $\sigma^2$, $\sigma_p^2$) are in log-duration space. The corresponding physical-duration estimate is $\hat{s}=\exp(\hat{y})$.

For the standard and test intervals, the posterior means are
\begin{align}
\hat{y}_s &= w\, m_s + (1-w)\,\mu_p,
\label{eq:posterior_standard} 
\end{align}
\begin{align}
\hat{y}_t &= w\, m_t + (1-w)\,\mu_p.
...
Decisions are based on the comparison of posterior log-duration estimates, such that the observer concludes that the test is longer when $\hat{y}_t > \hat{y}_s$. This is equivalent to comparing the corresponding physical-duration estimates because exponentiation is monotonic. Under the assumptions above, the posterior difference reduces to Eq.\ref{eq:posterior_difference}, so that the decision is equivalent to comparing the measurements directly, i.e., determining whether $m_t - m_s> 0$. From the modeler's perspective, the response probability is obtained by marginalizing over the measurement noise. Because $m_t$ and $m_s$ are independent random Gaussian variables with equal variance $\sigma^2$, their difference follows
\begin{equation}
m_t - m_s
\sim
\mathcal{N}\!\left(
...
The forced-fusion model assumes that participants produce an estimate of auditory log-duration by integrating visual and auditory measurements in log-duration space, weighted by their relative reliabilities. The precision-weighted Gaussian component of the common-cause posterior has mean and standard deviation
\begin{equation}
...
The implemented readout applies the same bounded uniform prior over log-duration, $[t_{\min},t_{\max}]$, used in the causal-inference model. The resulting posterior is a normal distribution with parameters $\mu_c$ and $\sigma_c$, truncated to these bounds. The forced-fusion estimate of log-duration is its Bayes least-squares (BLS) estimate, i.e., its posterior mean:
\begin{equation}
\hat{y}_{\text{fusion}}
=
\mu_c+\sigma_c
...
The probabilistic cue-switching model represents an alternative strategy. On each trial, the observer stochastically selects \textit{one} modality and bases the estimate entirely on that cue. For the selected modality, the implementation uses the BLS estimate of log-duration under the same bounded uniform prior: the mean of the single-cue truncated-normal posterior. The corresponding physical-duration estimate is obtained by exponentiation. Thus, the selected cue is decoded using a posterior mean rather than its raw measurement or a MAP estimate. This contrasts with averaging-based strategies that combine information from both modalities. The probability of using the visual cue is a free parameter of the model.
...
The bounded uniform prior above is used both to compute the marginal likelihoods and to decode log-duration under each causal structure. The readout is the Bayes least-squares estimate (posterior mean) under squared-error loss in log-duration space. Under a common cause, first define the precision-weighted Gaussian parameters
\begin{equation}
\mu_c = \frac{J_a m_a + J_v m_v}{J_a + J_v},
\qquad
\sigma_c^2 = \frac{1}{J_a + J_v}.
...
\end{equation}
The final auditory log-duration estimate is the posterior-probability-weighted average of these two BLS estimates:

\begin{equation}
\hat{y}_a = p(C=1 | m_a, m_v) \cdot \hat{y}_{C=1}
+ (1 - p(C=1 | m_a, m_v)) \cdot \hat{y}_{a,C=2}.
\end{equation}
Here, $J_a = 1/\sigma_a^2$ and $J_v = 1/\sigma_v^2$ denote sensory precisions (with $\sigma_a \in \{\sigma_{a,l}, \sigma_{a,h}\}$ depending on condition). All measurements, bounds, and estimates in these expressions are in log-duration space. The corresponding estimate in physical time is $\hat{s}_a=\exp(\hat{y}_a)$. In the segregated case ($C=2$), only the auditory posterior contributes to the task response, consistent with the instruction to judge auditory duration.
...
Observed audiovisual variability ($\sigma_{AV}$) was estimated by fitting psychometric functions to bimodal data. Predictions are computed separately for the high- and low-noise auditory conditions using the corresponding unimodal estimates ($\sigma_{a,h}$ and $\sigma_{a,l}$), and compared to the fitted bimodal variability.

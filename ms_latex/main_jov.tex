...
% such as \sf in captions. Use modern font encoding/text fonts when available
...
\end{align}
where $\hat S_{a,C=1}$ and $\hat S_{a,C=2}$ denote the interior MAP/maximum-likelihood estimates of the auditory stimulus under the common-cause and independent-causes assumptions, respectively.
...
We also examined whether the models captured the slope of the fitted psychometric functions. The slope of fitted psychometric functions $\sigma$ indexes sensory encoding precision in log-duration units, with larger values corresponding to lower encoding precision (Figure~\ref{fig:sigmaConflictModel}). The probabilistic cue-switching model appeared to track the conflict-dependent changes in slope more closely than the forced-fusion and causal-inference models. To quantify this, we computed the root mean squared error (RMSE) between the measured slopes and those predicted by each model in both noise conditions. RMSE was indeed lower for the cue-switching model (0.048 log units) than for the causal-inference model (0.066) or the forced-fusion model (0.072).
...
Beyond model comparison, the parameters of the favored model were themselves consistent with the cue-switching account. The fitted probability of relying on the visual duration ($p_v$) increased markedly with auditory noise, rising from a group mean of $0.17$ under low auditory noise to $0.53$ under high auditory noise (Wilcoxon signed-rank test, $p < 0.001$; an increase in $11$ of $11$ participants; Figure~\ref{fig:switchingProb}). Thus, observers were estimated to rely on the visual duration roughly half of the time when auditory reliability was low, but only about one in six trials when it was high, providing a concrete and consistent mechanism for the noise-dependent visual bias.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\linewidth]{assets/figures/switching_probability_by_noise.pdf}
    \caption{\textbf{Fitted switching probabilities increase with auditory noise.} Each colored thin line shows one participant's fitted probability of relying on the visual duration ($p_v$) in the low- and high-auditory-noise conditions, estimated from the probabilistic cue-switching model (free sensory-noise fits). The probability of relying on vision increased with auditory noise in all participants (group means $0.17 \rightarrow 0.53$; Wilcoxon signed-rank $p < 0.001$). Black diamonds and line show group means $\pm$ SEM.}
...
\end{align}
This formulation assumes that inference is performed in log-duration space, where sensory noise is Gaussian. Note that performing maximum a posteriori (MAP) integration in linear duration would not make the same predictions as the MAP solution computed in log duration and would not result in a weighted average of measurement and prior mean.
...
In the forced-fusion readout, we do not include an explicit duration-prior term; the estimate is defined as the reliability-weighted average of the auditory and visual measurements. A common prior applied to both intervals would not change the reliability-weighted mean comparison that determines the direction of the response, although it would change the posterior variance. In the fitted forced-fusion model, we therefore use the measurement-noise parameters ($\sigma_a$, $\sigma_v$) to determine the reliability weights, and do not include additional prior-mean or prior-variance parameters.
...
The bounded uniform prior above is used to compute the marginal likelihoods for the common- and separate-cause hypotheses. The final duration readout uses the interior MAP/maximum-likelihood estimate under each causal structure. Thus, the final auditory-duration estimate is a posterior-weighted average of the fused estimate under a common cause and the auditory estimate under separate causes:

\begin{equation}
\hat{s} = p(C=1 | m_a, m_v) \cdot \hat{s}_{C=1}^{\mathrm{MAP}} + (1 - p(C=1 | m_a, m_v)) \cdot \hat{s}_{a,C=2}^{\mathrm{MAP}},
\end{equation}
where $\hat{s}_{C=1}^{\mathrm{MAP}}$ is the reliability-weighted fused estimate (in log-duration space):
\begin{equation}
\hat{s}_{C=1}^{\mathrm{MAP}} = \frac{\sigma_v^2}{\sigma_a^2 + \sigma_v^2} m_a + \frac{\sigma_a^2}{\sigma_a^2 + \sigma_v^2} m_v 
= \frac{J_a}{J_a + J_v} m_a + \frac{J_v}{J_a + J_v} m_v,
\end{equation}
where $J_a = 1/\sigma_a^2$ and $J_v = 1/\sigma_v^2$ denote sensory precisions (with $\sigma_a \in \{\sigma_{a,l}, \sigma_{a,h}\}$ depending on condition). In the segregated case ($C=2$), the observer reports the auditory estimate
\begin{equation}
\hat{s}_{a,C=2}^{\mathrm{MAP}} = m_a,
\end{equation}
consistent with the task instruction to judge auditory duration. With a bounded uniform prior, these expressions correspond to the MAP/maximum-likelihood solution when the posterior mode lies within the prior support. We did not use a posterior-mean/Bayesian least-squares readout.
...
The model-recovery results reveal substantial differences in identifiability across models (Figure~\ref{fig:modelRecoveryResults}). Recovery was strongest for the probabilistic cue-switching strategy, which was correctly identified in 92\% of simulated datasets, indicating that this model generates relatively distinctive behavioral predictions. Recovery for the forced-fusion model was moderate (64\%), with appreciable confusion with the causal-inference model. In contrast, the causal-inference model showed poor mutual discriminability; 44\% of the time the recovered model was correct and also 44\% of the time the results were confused with the forced-fusion model. This pattern suggests that only the probabilistic cue-switching model generates distinguishable behavioral data.

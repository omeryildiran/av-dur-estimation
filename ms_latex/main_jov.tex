...
We also examined whether the models captured the width of the fitted psychometric functions. The psychometric funtions sigma parameter indexes sensory uncertainty in log-duration units, with larger values corresponding to shallower psychometric functions and lower precision (Figure~\ref{fig:sigmaConflictModel}). Descriptively, the probabilistic cue-switching model tracked conflict-dependent changes in psychometric-function width more closely than the forced-fusion and causal-inference models. Across the 14 group-averaged noise-by-conflict conditions, the model-data RMSE for psychometric-function width was lower for cue switching (0.048 log units) than for causal inference (0.066) or forced fusion (0.072). Because larger width values correspond to shallower psychometric-function slopes, this suggests that the switching model better captured changes in discrimination precision across conflict conditions.
...
\caption{
Conflict-dependent changes in psychometric-function width. Sigma estimates are shown in log-duration units as a function of audiovisual conflict for the low auditory-noise condition (\textbf{A}) and high auditory-noise condition (\textbf{B}), averaged over 11 participants. Red and blue points show empirical data (group mean $\pm$~SEM across participants). Other lines indicate model predictions for forced fusion, causal inference, and probabilistic cue switching, with model error bars showing $\pm$~SEM across participant fits. Larger sigma values indicate lower duration-discrimination precision.}
...
\begin{figure}
    ...
    \caption{This figure illustrates the relationship between conflict levels and PSE shifts across different auditory-noise conditions for each participant. Here, the $\sigma$ values were estimated using the bimodal data.}  

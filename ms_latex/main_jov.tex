...
\end{align}
where $\hat S_{a,C=1}$ and $\hat S_{a,C=2}$ denote the estimates of the auditory stimulus under the common-cause and independent-causes assumptions, respectively. This strategy produced an estimate that smoothly interpolated between fusion and segregation as a function of sensory conflict and uncertainty, resulting in graded biases toward the more reliable cue. Causal inference is performed in log-duration space, consistent with the assumed log-Gaussian sensory encoding and inference framework.
...
Figure~\ref{fig:pseConflictModel} shows model fits to the PSE-shift data. For ease of visualization, predictions of only three of the models are displayed: forced fusion, causal inference and probabilistic cue switching. All three models capture the overall pattern that perceived auditory duration was systematically biased toward visual duration and that the bias increased approximately linearly with audiovisual conflict magnitude. All three models also predict an overall larger bias under higher auditory noise. This convergence suggests that our data may be limited in the ability to adjudicate between these models. A comparison that includes all three models and fits to individual participants' data is presented in the Appendix (Figure~\ref{fig:AppendixConflictPSE_free}). 
...
The probabilistic cue-switching model provided the best fits for the largest number of participants (5 out of 11; Figure~\ref{fig:modelComparison}B). At the group level, it also yielded the lowest summed $\Delta$AIC across participants. However, the individual $\Delta$AIC values were very low, suggesting limited discriminability between integration strategies under this assumption.

\subsubsection{Group-level model preference and its reliability}
To summarize model preference across participants while accounting for between-participant variability, we computed the mean per-participant $\Delta$AIC relative to the forced-fusion model, with 95\% confidence intervals from a participant bootstrap (Figure~\ref{fig:meanDeltaAIC}). Under fixed sensory noise, both the probabilistic cue-switching model (mean $\Delta$AIC $= -40.8$, 95\% CI $[-70.1,\,-15.3]$) and the causal-inference model ($-18.5$, $[-43.0,\,-2.3]$) fit reliably better than forced fusion, with cue switching preferred overall; both confidence intervals excluded zero. Under free sensory noise, the same ordering held, but the differences were small and not statistically reliable (cue switching $-2.2$, $[-6.9,\,+1.8]$; causal inference $-0.9$, $[-2.7,\,+0.9]$), with both confidence intervals spanning zero.
...
\begin{figure}
    \centering
    \includegraphics[width=0.95\linewidth]{assets/figures/meanDeltaAIC_threeModels.pdf}
    \caption{\textbf{Group-level model preference with participant bootstrap.} Mean per-participant $\Delta$AIC relative to the forced-fusion model (baseline $=0$, not shown) under fixed (left) and free (right) sensory noise. Black markers show the mean across participants with 95\% confidence intervals from $10{,}000$ participant bootstrap resamples; coloured points show individual participants. Values below zero indicate a better fit than forced fusion. Under fixed sensory noise, the probabilistic cue-switching and causal-inference models fit reliably better than forced fusion; under free sensory noise the differences are small and the confidence intervals span zero.}
    \label{fig:meanDeltaAIC}
\end{figure}
...
= \frac{1}{\Delta_t}
\int_{t_{\min}}^{t_{\max}}
\mathcal{N}(m_a; y, \sigma_a^2)\\
\times \mathcal{N}(m_v; y, \sigma_v^2) \, dy.
\end{multline}
...
p(m_a | y_a) p(y_a) \, dy_a\right]\\
\times \left[\int_{t_{\min}}^{t_{\max}}
...
\begin{figure} [H]
    \centering
    \includegraphics[width=0.85\linewidth]{assets/figures/posterior_overlay_per_subject_v2.pdf}
...
To assess when the candidate models should be expected to become distinguishable, we performed an extended model-recovery analysis across simulated sensory-noise and conflict sampling regimes. We simulated datasets from the three candidate models while varying both the sensory-noise range and the maximum audiovisual conflict. Four sensory-noise regimes were considered: very low noise, $\sigma_a,\sigma_v \sim U[0.01,0.20]$; low noise, $\sigma_a,\sigma_v \sim U[0.20,0.40]$; medium noise, $\sigma_a,\sigma_v \sim U[0.30,0.70]$; and an empirical regime based on the range of fitted sensory-noise parameters. For the synthetic regimes, auditory and visual sensory noise parameters were sampled from the same range.

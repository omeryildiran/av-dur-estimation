...
We model the prior distribution of durations as a truncated uniform distribution over the range of stimulus durations presented in the experiment, $[t_{\min}, t_{\max}]$, providing finite support for the latent duration variable. 
...
\end{align}
where $\hat S_{a,C=1}$ and $\hat S_{a,C=2}$ denote the posterior-mean estimates of the auditory stimulus under the common-cause and separate-causes assumptions, respectively. Because the latent-duration prior has bounded uniform (boxcar) support, these are means of truncated-normal posteriors rather than maximum a posteriori estimates; their full expressions are given in the Appendix.
...
We assume that the participant uses a flat prior over the log-duration $y$ ranging over $[t_{\min}, t_{\max}]$ (where $t_{\min}$ and $t_{\max}$ denote the bounds of the log-duration range presented in the experiment). This prior is needed to normalize the integral below to compute the evidence for a common cause. The prior is
\begin{equation}
p(y) = \frac{1}{t_{\max} - t_{\min}}. 
\text{ for } t_{\min}<y<t_{\max}
\end{equation}
...
Importantly, the high recovery of the cue-switching model did not reflect a general tendency to over-select the most flexible model. Cue switching was selected for only $6$ of $50$ datasets generated by the forced-fusion model and by $6$ of $50$ datasets generated by the causal-inference model, but it was selected for $46$ of $50$ datasets generated by the cue-switching model. Its preference in the empirical data therefore reflects a genuine, recoverable behavioral signature rather than spurious model flexibility.
%%% --- end added section ---
...
When reading the cue-swictching row across columns, it is important not to treat all four panels as a single monotonic noise sweep. Across the three synthetic regimes, recovery of the cue-switching model declined steadily as sensory noise increased (for the $0.25\,\mathrm{s}$ conflict range, from $86\%$ in the very-low-noise regime to $48\%$ and $28\%$ in the low- and medium-noise regimes), with its simulated datasets increasingly misclassified as forced fusion. The empirical column does not continue this downward trend: because it was generated from the fitted participant-level parameters rather than a uniform noise range, it sits off the synthetic-noise axis, and its high recovery ($84\%$) reflects that the empirically fitted parameters occupy a regime in which cue switching produces a particularly distinctive behavioral signature. The apparent increase in cue-switching recovery from the medium-noise column to the empirical column is therefore a consequence of placing an off-axis regime adjacent to the synthetic gradient, not a genuine non-monotonic dependence on sensory noise.
...
\begin{figure}[H]
    ...
    \caption{\textbf{Model recovery across sensory-noise and conflict regimes.} Confusion matrices for the three candidate models: forced fusion, causal inference, and probabilistic cue switching. Rows indicate the generating model, and columns indicate the recovered model selected by AIC. Cell values show row-normalized recovery percentages, with diagonal entries corresponding to correct recovery. Panels compare four sensory-noise regimes and two maximum conflict ranges. Row~\textbf{A} (top) uses $\mathrm{conflict}_{\max}=0.45\,\mathrm{s}$, whereas row~\textbf{B} (bottom) uses $\mathrm{conflict}_{\max}=0.25\,\mathrm{s}$. Synthetic sensory-noise regimes were defined as $\sigma_a,\sigma_v \sim U[0.01,0.20]$, $U[0.20,0.40]$, and $U[0.30,0.70]$, corresponding to the first three columns from left to right. The empirical panel, highlighted in red, was computed from datasets simulated using the fitted participant-level parameters and then refitted with all candidate models. The upper-right confusion matrix was also computed from datasets simulated using the fitted participant-level parameters, except that a larger conflict range ($\pm0.45\,\mathrm{s}$) was simulated. Because the empirical (rightmost) column was generated from the fitted participant-level parameters rather than a synthetic noise range, it lies off the left-to-right noise gradient defined by the first three columns and should not be read as a continuation of it; its high cue-switching recovery therefore does not represent a non-monotonic reversal of the noise trend. Recovery generally decreased with increasing sensory noise and narrower conflict ranges. Under the empirical parameter regime, probabilistic cue switching showed excellent recovery, whereas forced fusion and causal inference exhibited greater mutual confusability.}        

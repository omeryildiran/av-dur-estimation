...
We did not include a bias parameter ($\mu$) in the main analyses. In a symmetric two-interval, forced-choice (2IFC) design, any additive bias in perceived log-duration would affect both the standard and test intervals equally and therefore cancel in the comparison. Consequently, when  the standard($t$) and the test($t$) durations are identical, the model predicts $P(\text{``test longer''}) = 0.5$ by symmetry, and no bias term is theoretically required. 
...
\end{equation}
where $t$ and $s$ denote the test and standard durations, respectively, and $\Phi$ is the standard normal cumulative distribution function. Figure~\ref{fig:unimodal_data_fits} shows the unimodal duration-discrimination thresholds (Weber fraction; A) and the psychometric-function fits (B) for the low-noise auditory, visual, and high-noise auditory conditions.
...
\begin{figure}
    ...
    \caption{ \textbf{PSE shifts and psychometric curve fits for the cross-modal duration 
    comparison.} \textbf{A} Point of subjective equality (PSE) shifts for the low-noise auditory (red) and high-noise auditory (blue) conditions. Superimposed are the individual participant data points (horizontally jittered). The PSE shift is the difference between the standard duration (500~ms) and the auditory duration that is perceived as identical to the standard. \textbf{B} Psychometric functions (dots) and model fits (solid lines), showing $p($``auditory test longer''$)$. Binary data were grouped into 7 equally-spaced bins between 200 and 800~ms (error bars: SEM).
    For this plot we pooled the data across all participants then fitted with log-normal cumulative distribution functions (parameters: $\lambda$: lapse rate; $\mu$: PSE, $\sigma$: log-space noise). Solid lines show fit log-normal psychometric functions. PSEs are shifted leftward in both conditions, indicating a systematic overestimation of auditory relative to visual duration.}
...
\label{eq:pc}
\end{align}
where $C = 1$ denotes a common cause. The observer then forms a posterior over the task-relevant variable (here, the auditory stimulus $s_a$) by averaging over two posteriors:
\begin{align}
...
\end{align}
in which $p(C = 2 \mid m_v, m_a)$ indicates the posterior probability of separate causes. $p(s_a \mid m_v, m_a, C=1)$ and $p(s_a \mid m_a,C=2)$ specify the posteriors over $s_a$ under each causal scenario. The posterior probability of a common cause was computed as:
...
We tested three causal-inference models, in which the posterior probability of a common cause$p(C=1|m_a,m_v)$ was computed in the same way. However, the models differed in how this posterior probability was applied to form the final duration estimate used for the decision. Specifically, the models used the posterior probability to (i) average the estimates under the common- and separate-cause hypotheses weighted by their posterior probabilities, (ii) select the estimate associated with the more probable causal structure, or (iii) stochastically sample between these estimates according to the posterior probability. Each model included a prior probability of common cause, $p_c$, which was fitted to the data.
...
\caption{
Conflict-dependent biases in auditory duration perception. Point of subjective equality (PSE) shifts as a function of audiovisual conflict for the low auditory-noise condition (\textbf{A}) and high auditory-noise condition (\textbf{B}), averaged over 11 participants. Positive values indicate biases toward longer perceived auditory duration. Red and blue show empirical data (group mean $\pm$ SEM across 11 participants). Other lines indicate model predictions for forced fusion, causal model averaging, and probabilistic cue switching. The model predictions are jittered horizontally for readability. All model predictions were generated using sensory-noise parameters fitted using the bimodal data. For models, error bars reflect SEM across participant fits.}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{assets/figures/modelComparisonTables_combined_1.pdf}
    \caption{
...
\begin{figure}[tbp]
  ...
  \caption{\textbf{Posterior probability of a common cause across conflict levels.}
    Log likelihoods and posterior probability of a common cause for an example participant, plotted as a function of audiovisual duration conflict. Top row: low-auditory-noise condition; bottom row: high-auditory-noise condition. Left column: log likelihood of the measurements under a common cause, $\log P(m_a, m_v \mid C{=}1)$ (blue) and under separate causes, $\log P(m_a, m_v \mid C{=}2)$ (red), plotted on the same axes so that their relative magnitudes can be compared directly. Right column: Posterior probability of a common cause, $P(C{=}1 \mid m_a, m_v)$, with the dotted line marking the fitted prior $p_c$. All quantities were computed via Monte Carlo simulation (500 samples per conflict level) using participant-specific sensory-noise estimates ($\sigma_{a,l}$, $\sigma_{a,h}$, $\sigma_v$) and common-cause prior ($p_c$) obtained from the log-normal causal-inference model fit. Because the two log likelihoods decrease at comparable rates as conflict grows, their ratio remains approximately constant, and the posterior stays well above 0.5 across all conflict levels especially in the high auditory-noise condition. This indicates that, for this participant, the observer's fitted parameters are consistent with treating the auditory and visual signals as originating from a common source regardless of the imposed conflict, explaining why the causal-inference and forced-fusion models produce similar predictions.}
    \label{fig:posterior_conflict}
\end{figure}

% This finding has important implications for model identifiability. The signature of causal inference---a non-linear breakdown of integration at large conflicts---requires that the posterior probability of common cause drop substantially below the integration threshold. With the sensory noise levels observed in duration perception, achieving this would require conflicts far exceeding the $\pm250$~ms range tested here, conflicts that would be perceptually implausible and likely trigger explicit awareness of cue mismatch.
...
\textbf{Causal inference versus heuristic strategies}
...
The lack of evidence for causal inference may not be interpreted as evidence against its plausibility. Our results highlight a fundamental limitation in discriminating between integration strategies when the measurement noise is high enough. Under these conditions, the inference of separate causes is suppressed even with large cue conflicts. As a result, localization biases toward the visual stimulus continue to increase linearly with increasing cue conflict. Under such conditions, heuristic strategies that stochastically select between cues can closely approximate the predictions of more complex inferential models. Recent work has shown that probabilistic switching strategies can reproduce behavioral patterns that are often interpreted as Bayesian integration, even in the absence of explicit inference over causal structure \citep{ni2024computational}. This perspective aligns with our finding that multiple integration strategies produce near-indistinguishable behavioral signatures in audiovisual-duration perception.

\textbf{Limits of identifiability and factors shaping audiovisual duration perception}
...
\textbf{Asymmetry between shorter and longer audiovisual bias}
...
We adapted the stimuli from Hartcher and colleagues \citeyearpar{hartcher2014duration} and Shi and colleagues \citeyearpar{shi2013reducing} in which they created their stimuli by adding a pure-tone signal to a pink-noise background and manipulating duration uncertainty by varying the signal-to-noise ratio. Compared to their manipulation, in our stimulus, we generated white noises for both the background noise intervals and signal intervals, then differentiated the signal from the noise using different broadband filters to ensure that the signal was always detectable even when the noise was louder than the signal. 

The auditory stimuli and the background noise were both filtered white-noise signals sampled at 48~kHz. Each interval's signal was a noise burst amplitude modulated by a raised cosine (5~ms cosine ramp up, flat peak, cosine ramp down). The nominal duration of the sound was the duration at peak amplitude. The signal was embedded in a continuous background noise that spanned the entire trial. 

We manipulated auditory uncertainty by varying the relative amplitude of the continuous background noise with respect to the signal waveform before final mixing. In the low-noise condition, background amplitude was scaled to approximately 0.1 times the signal waveform amplitude, whereas in the high-noise condition it was scaled to approximately 1.2 times that amplitude. The final mixed waveform was then peak-normalized for playback.


To keep the “signal” perceptually distinct even with the high noise level, we applied different band-pass filters to the signal and background tracks. The background noise was band-pass filtered with cutoffs at 10 and 610~Hz. The stimuli (test and standard) were band-pass filtered from 150–775~Hz to make the signal more distinguishable from the background. The two tracks were then summed to create the final waveform (background + signal), which was peak-normalized prior to playback to ensure consistent overall amplitude across trials. All filtering was implemented using second-order sections (SOS) Butterworth filters with a filter order of 4.

Auditory stimuli were calibrated using an iPhone 11 with "Decibel: db sound level meter" app positioned at the participant's head location. The signal (test and standard stimuli) had an RMS intensity of approximately 77 dB SPL. In the low-noise condition, the continuous background noise was presented at 60 dB SPL (signal-to-noise ratio, SNR ≈ +17 dB). In the high-noise condition, the background noise was presented at 78.6 dB SPL (SNR ≈ -1.6 dB). All sound intensities are reported as A-weighted RMS levels.
...
In both tasks, the standard stimulus duration was fixed at 500~ms. The duration of the test auditory stimulus varied (between 25 and 975~ms) from trial to trial. The waveform of a example stimulus in the low and high auditory-noise conditions is shown in Figure~\ref{fig:unimodal_Timelines}.
...
Each participant carried out three sessions, with audiovisual duration conflicts of (0, --167, 250~ms), (-83,167~ms),  and (-250, 83~ms), respectively. The order of these sessions was counterbalanced across participants.  As in the unimodal condition, we have also included easy lapse-rate trials in each experimental block. There were 14 lapse-rate trials per condition (noise-level x conflict). There were two interleaved 1-Down 2-Up and two 2-Down 1-Up staircases, each with 70~trials for each condition and staircase. In total, there were 2156 trials for the main task.
...
This starting point assumed zero audiovisual conflict and no participant-specific visual bias. To improve the accuracy of the initial estimates, we adjusted the staircase for each participant by incorporating their measured cross-modal bias (i.e., point of subjective equality). This bias correction ensured that staircases began near perceptual threshold for each participant. Additionally, we constrained the range of the test durations so that $|\Delta| \leq 0.95 \times$ the standard duration, preventing the test interval from becoming implausibly short or long.
...
\end{align}
This formulation assumes that inference is performed in log-duration space, where sensory noise is approximately Gaussian. Performing integration in linear time would not correspond to the MAP solution under this generative model, but rather to a mis-specified inference strategy (cf. log–linear mismatch model below).
...
Under the log-space Gaussian measurement model,
\begin{align}
m_t &\sim \mathcal{N}(\log s_t, \sigma^2), \\
m_s &\sim \mathcal{N}(\log s_s, \sigma^2),
\end{align}
where $s_t$ and $s_s$ denote the physical durations of the test and standard intervals.
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth,height=1\textheight,keepaspectratio]{assets/figures/pse_vs_conflict_compact_free_sigma.pdf}
    \caption{This figure illustrates the relationship between conflict levels and PSE shifts across different auditory-noise conditions for each participant. Here, the $\sigma$ values were estimated using the bimodal data. Error bars represent the mean ± standard deviation from bootstrapped data.}  
...
\begin{figure}
    ...
    \caption{This is exactly the same figure as the Figure~\ref{fig:AppendixConflictPSE_free}, except that here the models are fitted using the sensory-noise parameters ($\sigma_{a,l},\sigma_{a,h},\sigma_{v}$) that were estimated based on the psychometric fits to the unimodal data.}
...
\begin{figure}[tbp]
    ...
    \caption{\textbf{Log likelihoods and posterior probability of a common cause for each participant.}
...
Panels show results for each noise condition as well as averages across conditions. Each point corresponds to an individual participant (color-coded). The dashed unity line ($\sigma_{\mathrm{opt}} = \sigma_{AV}$) indicates perfect agreement between predicted and observed variability. Systematic deviations from this line reflect departures from optimal integration based solely on unimodal noise estimates.
...
\begin{figure}
    ...
    \caption{ \textbf{Parameter-recovery analysis for model validation.} Each row corresponds to a generative model, and each column to a model parameter. For each model, synthetic datasets ($N = 200$) were generated using parameters sampled from the fitted group-level distributions, and the same model was then re-fitted to recover parameters. Scatter plots show true (x-axis) versus recovered (y-axis) parameter values. The black dashed diagonal indicates perfect recovery (slope $=1$), and the gray shaded region denotes a $\pm 5\%$ tolerance band. Insets report Pearson correlation ($r$) and RMSE. Across rows, the models are forced fusion, model averaging, probability matching, model selection, and probabilistic cue switching. Lapse-rate ($\lambda$) and sensory-noise parameters ($\sigma_{a,l}$, $\sigma_{a,h}$, $\sigma_v$) are generally recovered with high accuracy, indicating good identifiability of the core sensory parameters. Recovery of the causal prior parameter ($p_c$) is weaker in the causal-inference models, and the switching parameters ($p_{v,l}$, $p_{v,h}$) in the probabilistic cue-switching model show moderate variability, consistent with increased model flexibility and parameter trade-offs. Overall, parameters that directly shape the psychometric function are robustly recovered, whereas higher-level inference parameters are less constrained by the data. }
    \label{fig:parameterRecovery}
\end{figure}
To validate the identifiability and distinguishability of competing models, we conducted comprehensive model-recovery and parameter-recovery analyses. These analyses ensured that (1)~each model's parameters can be reliably recovered from data generated by that model (parameter recovery, Figure~\ref{fig:parameterRecovery}), and (2)~different models can be correctly distinguished when applied to data generated by competing models (Figure~\ref{fig:modelRecoveryResults}).

The parameter recovery was carried out as follows. For each participant and model combination:
\begin{enumerate}
    \item \textbf{Original parameter estimation}: We first fitted each model to the empirical data to obtain best-fitting parameters.
    \item \textbf{Synthetic data generation}: Using the fitted parameters as ground truth, we simulated 200 synthetic datasets by sampling responses from the model's predicted psychometric function. Each synthetic dataset matched the original experimental design (same stimulus conditions, trial counts, and structure).
    \item \textbf{Parameter re-estimation}: Each model was re-fitted to each synthetic dataset using the same optimization procedure applied to empirical data (maximum likelihood estimation with 2000 Monte Carlo simulations 10 random starting points).
\end{enumerate}
...
To perform this analysis, after fitting each candidate model we used the fitted parameter values to generate 200 synthetic datasets per model. Parameters were sampled from group-level distributions estimated from the 11 participants included in the main analyses.
We fitted again to each of the generated datasets to each model and recorded their AIC values.

...
if __name__ == "__main__":
    ...
    import sys
    dataFile = sys.argv[0] if len(sys.argv) > 1 else "mt_all.csv"
    modelName = sys.argv[1] if len(sys.argv) > 2 else "lognorm"
    nSimul = int(sys.argv[2]) if len(sys.argv) > 3 else 2000
    optimMethod = sys.argv[3] if len(sys.argv) > 4 else "bads"
    nStarts = int(sys.argv[4]) if len(sys.argv) > 5 else 1
    print(f"Data file: {dataFile}, Model: {modelName}, Simulations: {nSimul}, Optimizer: {optimMethod}, Starts: {nStarts}")
